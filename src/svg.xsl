...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template name="test">
        ...
        <!-- SIMPLE PATH -->
        <xsl:if test="not(name($station[1]/..)='subpath')">
            <g transform="translate({$x_coordinate},{$y_coordinate})">
                <!-- if the next one exists and is not a subpath, draw a line -->
                <!--  <xsl:if test="$station[2] and not(name($station[2]/../..)='subpath')"> -->
                <xsl:value-of select="$station[1]/ancestor::line/@name"/>
                <xsl:value-of select="$station[1]/../../@name"/>
                <xsl:value-of select="$station[1]/preceding::station/../../@name"/>
                <xsl:if test="$station[2]">
                ...
                <xsl:if test="boolean(($station[1]/../preceding::bifur)) 
                and (count($station[1]/preceding-sibling::station))=0
                and ($station[1]/ancestor::line/@name) = ($station[1]/preceding::station/ancestor::line/@name)">
                    <path d="M 0 0 H {-$pathWidth}" style="stroke:#006fb6;stroke-width:8" />
                </xsl:if>
                <!-- station name above circle -->
                <xsl:choose>
                    <xsl:when test ="$station[1]/changes/changeline">
                        <circle  r="{$circleRadius}" fill="white" stroke="black" stroke-width="3" />
                        <xsl:for-each select="$station[1]/changes/changeline">
        ...
        <xsl:if test="(count($station[1]/../preceding-sibling::subpath)+1) = 1 
            ...
            <g transform="translate({$x_coordinate},{$y_coordinate -$bifurHeigh})">
                ...
                <xsl:if test="$station[2] and (name($station[2]/..)='subpath') and (count($station[2]/../preceding-sibling::subpath)+1) = 1 ">
                    <path d="M 0 0 H {$pathWidth}" style="stroke:#006fb6;stroke-width:8" />
                </xsl:if>
                <!-- DRAW PATH TO THE LEFT OF FIRST STATION OF SUBPATH IF THERE IS A STATION BEFORE -->
                <xsl:if test="boolean(($station[1]/../../preceding::station)) and (count($station[1]/preceding-sibling::station)+1) = 1
                and (((count($station[1]/../../preceding-sibling::fpath)) != 0) 
                or (((count($station[1]/../../preceding-sibling::lpath)) != 0)))">
                    <path d="M 0 0 H {-$pathWidth}" style="stroke:#006fb6;stroke-width:8" />
                ...
                <xsl:if test="boolean(($station[1]/../../following::station)) and
                (count($station[1]/../preceding-sibling::subpath)+1) != (count($station[2]/../preceding-sibling::subpath)+1)
                and ($station[1]/ancestor::line/@name) = ($station[1]/../../following::station/ancestor::line/@name)">
                    <path d="M 0 0 H {$pathWidth}" style="stroke:#006fb6;stroke-width:8" />
                    <path d="M {$pathWidth} 0  L {$jumpBifur} {$bifurHeigh}" style="stroke:#006fb6;stroke-width:8" />
                </xsl:if>
                <!--  -->
        ...
        <xsl:if test="(count($station[1]/../preceding-sibling::subpath)+1) = 2 
            ...
            ">
            <g transform="translate({$x_coordinate},{$y_coordinate+$bifurHeigh})">
                <xsl:if test="$station[2] and (name($station[2]/..)='subpath') ">
                    <path d="M 0 0 H {$pathWidth}" style="stroke:#006fb6;stroke-width:8" />
                </xsl:if>
                <!-- DRAW PATH TO THE LEFT OF FIRST STATION OF SUBPATH IF THERE IS A STATION BEFORE -->
                <xsl:if test="boolean(($station[1]/../../preceding::station)) and (count($station[1]/preceding-sibling::station)+1) = 1 
                ...
                <xsl:if test="boolean(($station[1]/../../following::station)) and (boolean($station[1]/../ancestor::bifur) != boolean($station[2]/../ancestor::bifur))
                and (($station[1]/following::station/ancestor::line/@name) =($station[1]/ancestor::line/@name))">
        ...
        <xsl:if test="count($station) > 1">
            <xsl:call-template name="test">
                ...
                <xsl:with-param name="x_coordinate">
                    <xsl:choose>
                        <xsl:when test="((count($station[2]/preceding-sibling::station) = 0 and (count($station[2]/../preceding-sibling::subpath)+1)=2))">
                            <xsl:choose>
                                <xsl:when test="($station[2]/ancestor::line/@name) = ($station[2]/../../following::station/ancestor::line/@name)">
                                    <xsl:value-of select="$x_coordinate - (100*(count($station[2]/following-sibling::station)))"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="$x_coordinate - (100*(count($station[2]/preceding::subpath[1]//following-sibling::station)-1))"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="
                            if(boolean($station[1]/../ancestor::bifur) != boolean($station[2]/../ancestor::bifur))
